...
class WGAN():
    def __init__(self,i, n_features,latent_space,BATCH_SIZE,n_var ,use_bias, lr =0.0001, activation1 = 'relu', activation2='tanh'): #relu,tanh #lr=0.0001 
        ...
        self.generator = build_generator(self.latent_space, self.n_var, self.n_features, self.use_bias, activation1=activation1, activation2=activation2)
        self.critic = build_critic(self.latent_space, self.n_features, self.use_bias, activation1=activation1)
        ...
        self.optimizer = tf.keras.optimizers.Adam(5e-7)#5e-4,6e-4 good
    ...
    def train(self, dataset, epochs, scaler, scaled, X_train, y_train):
        ...
        for epoch in range(epochs):
            ...
            if ((epoch+1) % 5000 == 0):
                #save the model
                self.wgan.save('./GANs/Models/GAN_'+'v'+str(i)+'_epoch='+str(epoch+1)+'.h5')
                self.generator.save('GANS/Models/generator_'+'v'+str(i)+'_epoch='+str(epoch+1))
                self.critic.save('GANS/Models/discriminator_'+'v'+str(i)+'_epoch='+str(epoch+1))
