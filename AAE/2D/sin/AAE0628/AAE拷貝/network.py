...
def build_encoder(Z, nodes, n_features):
    ...
    enc = Dense(nodes, use_bias=False)(input_enc)#16
    ...
    enc = Dense(nodes*4, use_bias=False)(enc)#(4,8) #
    enc = BatchNormalization()(enc)#nodes*4 #
    enc = ReLU()(enc) #
    
    enc = Dense(nodes, use_bias=False)(enc)#(4,2)
    enc = BatchNormalization()(enc)
    enc = ReLU()(enc)
...
def build_decoder(Z, var, n_features, use_bias):
    ...
    dec = Dense(var*15,activation='relu', use_bias=use_bias)(input_dec)
    dec = BatchNormalization()(dec)
    
    dec = Dense(var*5,activation='relu', use_bias=use_bias)(dec)
    dec = BatchNormalization()(dec)
...
def build_discriminator(Z, nodes):
    ...
    disc = Dense(nodes, use_bias=False)(in_disc)
    disc = BatchNormalization()(disc)
    disc = ReLU()(disc)
    #disc = Reshape((a, b))(disc)
    
    disc = Dense(nodes*4, use_bias=False)(disc)
    disc = BatchNormalization()(disc)
    disc = ReLU()(disc)
    
    disc = Dense(nodes*2, use_bias=False)(disc)
    disc = BatchNormalization()(disc)
    disc = ReLU()(disc)
