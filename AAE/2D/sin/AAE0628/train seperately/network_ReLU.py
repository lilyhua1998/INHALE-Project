...
def build_decoder(Z, nodes, n_features):
    ...
    dec = Dense(a*b, use_bias=False)(input_dec)
    dec = Reshape((a, b))(dec)#(4,2)
    

    dec = Conv1DTranspose(a*b, kernel_size=1, strides=1, padding='same', use_bias=False)(dec)#(4,2)
    dec = ReLU()(dec)
    dec = BatchNormalization()(dec)
    
    dec = Conv1DTranspose(nodes*4, kernel_size=1, strides=1, padding='same', use_bias=False)(dec)#(4,8)
    dec = ReLU()(dec)
    dec = BatchNormalization()(dec)
    
    dec = Conv1DTranspose(nodes*2, kernel_size=1, strides=1, padding='same', use_bias=False)(dec)#(4,4)
    ...
    output_dec = Dense(n_features, activation='tanh')(dec)
