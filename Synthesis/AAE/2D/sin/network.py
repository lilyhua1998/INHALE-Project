...
def build_decoder(Z, var, n_features):

    # Input to the decoder is the latent space from the encoder
    input_dec = Input(shape=(Z,))#Z
    
    dec = Dense(var*15,activation='relu', use_bias=False)(input_dec)
    dec = BatchNormalization()(dec)
    
    dec = Dense(var*5,activation='relu')(dec)
    dec = BatchNormalization()(dec)
    
    dec = Dense(var*5,activation='relu')(dec)
    dec = BatchNormalization()(dec)
    
    output_dec = Dense(n_features, activation='tanh', use_bias=False)(dec)
