...
def build_generator(latent_space, n_features):
    ...
    model.add(Conv1DTranspose(n_features*8, kernel_size=1, strides=1, padding='same', use_bias=True))#*4
    model.add(BatchNormalization())
    model.add(ReLU())

    model.add(Conv1DTranspose(n_features*4, kernel_size=1, strides=1, padding='same', use_bias=True))
    model.add(BatchNormalization())
    model.add(ReLU())
...
def build_discriminator(n_features,latent_space,BATCH_SIZE):
    ...
    model.add(Conv1D(n_features*8, kernel_size=1, strides=1, padding='same'))
    model.add(BatchNormalization())
    model.add(LeakyReLU())

    model.add(Conv1D(n_features*4, kernel_size=1, strides=1, padding='same'))
    model.add(BatchNormalization())
    model.add(LeakyReLU())
