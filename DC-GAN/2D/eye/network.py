...
def build_generator(latent_space,nodes, n_features):
    ...
    model.add(Conv1DTranspose(nodes*4, kernel_size=1, strides=1, padding='same', use_bias=False))#*4
    model.add(BatchNormalization())
    model.add(LeakyReLU())

    model.add(Conv1DTranspose(nodes*2, kernel_size=1, strides=1, padding='same', use_bias=False))
    model.add(BatchNormalization())
    model.add(LeakyReLU())
...
def build_discriminator(n_features,nodes,latent_space,BATCH_SIZE):
    ...
    model.add(Conv1D(nodes*4, kernel_size=1, strides=1, padding='same'))
    model.add(BatchNormalization())
    model.add(LeakyReLU())

    model.add(Conv1D(nodes*2, kernel_size=1, strides=1, padding='same'))
    model.add(BatchNormalization())
    model.add(LeakyReLU())
